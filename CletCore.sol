...
contract CLETCORE is Ownable {
    ...
    mapping(address => string[]) private address_OwnedNames;
        ...
        returns (string[] memory)
    {
        ...
        for (
            ...
            if (!address_OwnedNames[_owner][index].isEqual("")) {
                data[index] = address_OwnedNames[_owner][index];
            }
        }
        return data;
    }
    ...
    function validatorRelease(address _address, string memory _name)
        public
        isValidator
    {
        doRelease(_address, _name);
    ...
    function doRelease(address _address, string memory _name)
        private
        noNull(_name)
    {
    ...
    function releaseListed(string memory _name, address _address)
        public
        onlyOwner
    {
    ...
    function validatorReleaseListed(string memory _name, address _address)
        public
        isValidator
    {
        doReleaseListed(_name, _address);
    ...
    function doReleaseListed(string memory _name, address _address)
        private
        noNull(_name)
    {
        CletName storage cletname = s_CletNames[name_ToID[_name]];
